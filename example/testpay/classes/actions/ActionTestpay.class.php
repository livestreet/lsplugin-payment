    ...
    protected function EventIndex()
    {
        ...
        if (isPost()) {
            ...
            if (!(is_numeric($iNumber) and $iNumber >= 1 and $iNumber <= 6)) {
                $this->Message_AddError('Нужно выбрать верный номер бублика - от 1 до 6');
                return;
            }

            $fSum = getRequest('bublik_sum');
            if (!func_check($fSum, 'float')) {
                $this->Message_AddError('Неверная сумма за бублик, необходимо ввести число (можно дробное)');
                return;
            }
            ...
            if ($iError !== 0) {
                $this->Message_AddError('При создании платежа возникла ошибка, смотри логи плагина payment');
            }
        }
    }
}