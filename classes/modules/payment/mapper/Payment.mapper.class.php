...
class PluginPayment_ModulePayment_MapperPayment extends Mapper
{
    ...
    public function AddPayment($oPayment)
    {
		...
		";
        if ($iId = $this->oDb->query($sql, $oPayment->getKey(), $oPayment->getType(), $oPayment->getSum(), $oPayment->getCurrencyId(), $oPayment->getDateAdd(),
            $oPayment->getIp(), $oPayment->getState())
        ) {
            return $iId;
        }
        return false;
    ...
    public function AddW1($oW1)
    {
		...
		";
        if ($this->oDb->query($sql, $oW1->getPaymentId(), $oW1->getWmiMerchantId(), $oW1->getWmiPaymentAmount(), $oW1->getWmiCurrencyId(),
                $oW1->getWmiToUserId(),
    ...
    public function AddMaster($oWm)
    {
		...
		";
        if ($this->oDb->query($sql, $oWm->getPaymentId(), $oWm->getLmiMerchantId(), $oWm->getLmiPaymentNo(), $oWm->getLmiSysPaymentId(),
                $oWm->getLmiSysPaymentDate(),
                $oWm->getLmiPaymentAmount(), $oWm->getLmiCurrency(), $oWm->getLmiPaidAmount(), $oWm->getLmiPaidCurrency(), $oWm->getLmiPaymentSystem(),
                $oWm->getLmiSimMode(), $oWm->getLmiPaymentDesc()) === 0
        ) {
            return true;
        }
        return false;
