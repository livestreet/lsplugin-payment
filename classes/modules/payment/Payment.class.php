...
class PluginPayment_ModulePayment extends Module
{
    ...
    public function LogError($iNumberError, $var = null)
    {
        if (Config::Get('plugin.payment.logs.error')) {
            $this->Logger_Error('Payment error: ' . $iNumberError, array('context' => $var), 'payment');
        }
        return $iNumberError;
    }
    ...
    public function MakePaymentSuccess($oPayment)
    {
        if ($oTarget = $this->GetTargetByPaymentId($oPayment->getId())) {
            ...
            $sMethod = 'MakePaymentSuccessTarget' . func_camelize($oTarget->getTargetType());
            if (method_exists($this, $sMethod)) {
    ...
    {
        if ($oTarget = $this->GetTargetByPaymentId($oPayment->getId())) {
            $sMethod = 'ProcessPaymentSuccessTarget' . func_camelize($oTarget->getTargetType());
            if (method_exists($this, $sMethod)) {
                return $this->$sMethod ($oPayment, $oTarget);
            }
        }
    ...
    {
        if ($oTarget = $this->GetTargetByPaymentId($oPayment->getId())) {
            $sMethod = 'ProcessPaymentFailTarget' . func_camelize($oTarget->getTargetType());
            if (method_exists($this, $sMethod)) {
                return $this->$sMethod ($oPayment, $oTarget);
            }
        }
    ...
    {
        if ($oTarget = $this->GetTargetByPaymentId($oPayment->getId())) {
            $sMethod = 'CheckAccessForPaymentTarget' . func_camelize($oTarget->getTargetType());
            if (method_exists($this, $sMethod)) {
                return $this->$sMethod ($oPayment, $oTarget);
            }
        }
    ...
    public function GetTargetInfo($sTargetType, $iTargetId)
    {
        ...
        $aReturn = array();
        $sMethod = 'GetTargetInfo' . func_camelize($sTargetType);
        if (method_exists($this, $sMethod)) {
    ...
    public function PreResultMaster()
    {
        ...
        if (Config::Get('plugin.payment.master.mid') != getRequest('LMI_MERCHANT_ID', null, 'post')) {
            return $this->LogError(self::PAYMENT_ERROR_MASTER_PRERESULT_WID,
                array(Config::Get('plugin.payment.master.mid'), getRequest('LMI_MERCHANT_ID', null, 'post'), $oPayment));
        }
        /**
         * Проверяем состояние нового платежа
         */
    ...
    public function ResultMaster()
    {
        ...
        if (Config::Get('plugin.payment.master.mid') != getRequest('LMI_MERCHANT_ID', null, 'post')) {
            return $this->LogError(self::PAYMENT_ERROR_MASTER_RESULT_WID,
                array(Config::Get('plugin.payment.master.mid'), getRequest('LMI_MERCHANT_ID', null, 'post'), $oPayment));
        }
        /**
         * Проверяем наличие предварительного запроса
    ...
    public function ResultRobox()
    {
         ...
         * Проверяем контрольную сумму
         */
        $sSig = md5(getRequest('OutSum', null,
                'post') . ":{$oPayment->getId()}:" . Config::Get('plugin.payment.robox.password_2') . ":shp_key={$oPayment->getKey()}");
    ...
    public function SuccessRobox()
    {
        ...
        $sSig = md5(getRequest('OutSum', null,
                'post') . ":{$oPayment->getId()}:" . Config::Get('plugin.payment.robox.password_1') . ":shp_key={$oPayment->getKey()}");
        if ($sSig != strtolower(getRequest('SignatureValue', null, 'post'))) {
