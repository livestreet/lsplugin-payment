...
class PluginPayment_ActionPayment extends ActionPlugin
{
    ...
    protected function EventAjaxProcessing()
    {
        ...
        if (!($oPayment = $this->PluginPayment_Payment_GetPaymentById(getRequest('payment_id', null, 'post')))) {
            $this->Message_AddErrorSingle($this->Lang_Get('plugin.payment.error_payment_id'));
            return;
        }
        ...
        if ($oPayment->getState() != PluginPayment_ModulePayment::PAYMENT_STATE_NEW) {
            $this->Message_AddErrorSingle($this->Lang_Get('plugin.payment.error_state'));
            return;
        }
        ...
        if (!($oTarget = $this->PluginPayment_Payment_GetTargetByPaymentId($oPayment->getId()))) {
            $this->Message_AddErrorSingle($this->Lang_Get('plugin.payment.error_target'));
            return;
        }
        ...
        if (!$this->PluginPayment_Payment_CheckAccessForPayment($oPayment)) {
            $this->Message_AddErrorSingle($this->Lang_Get('plugin.payment.error_payment_not_access'));
            return;
        }
        ...
        if (!in_array($sType, $aTypeAvailable)) {
            $this->Message_AddErrorSingle($this->Lang_Get('plugin.payment.error_payment_available_type'));
            return;
        }
        ...
        if ($oPayment->getType() == PluginPayment_ModulePayment::PAYMENT_TYPE_WM) {
            ...
            $oViewerLocal->Assign('SignatureValue',
                md5(Config::Get('plugin.payment.robox.login') . ":{$oPayment->getSum()}:{$oPayment->getId()}:" . Config::Get('plugin.payment.robox.password_1') . ":shp_key={$oPayment->getKey()}"));
